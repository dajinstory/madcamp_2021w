...
def vis_parsing_maps(im, parsing_anno, size_orig, stride, save_im=False, save_path='results/seg_result.jpg'):
    ...
    # Save result or not
    if save_im:
        vis_parsing_anno = Image.fromarray(vis_parsing_anno).resize(size_orig, Image.BICUBIC)
        vis_im = Image.fromarray(vis_im).resize(size_orig, Image.BICUBIC)
        vis_parsing_anno.save(save_path[:-4] +'.png')
        vis_im.save(save_path[:-4] +'.jpg')
        
################################################## DL_UTILS ####################################################        
        
def _det_image(sFilename, rFilename):
    myimg_orig = Image.open(sFilename)
    mybboxs, mylandmarks = detect_faces(myimg_orig)

    # Create Square
    res_img = myimg_orig.copy()
    draw = ImageDraw.Draw(res_img)
    for mybbox in mybboxs:
        x0, y0, x1, y1 = mybbox[:4]
        draw.line((x0, y0, x1, y0), width=10, fill=128)
        draw.line((x0, y0, x0, y1), width=10, fill=128)
        draw.line((x1, y1, x1, y0), width=10, fill=128)
        draw.line((x1, y1, x0, y1), width=10, fill=128)
    
    res_img.save(rFilename)
    return rFilename
    
def _sod_image(sFilename, rFilename):
    myimg_orig = Image.open(sFilename)
    mysods = nuki_faces(
        myimg_orig, 
        np.array([
            [0,0,myimg_orig.size[0], myimg_orig.size[1]]
        ]))
    mysod = mysods[0]

    res_img = np.asarray(myimg_orig.copy()).copy()
    res_img *= np.expand_dims(mysod, 2)
    res_img = Image.fromarray(res_img)
    res_img.save(rFilename)
    return rFilename

def _seg_image(sFilename, rFilename):
    myimg_orig = Image.open(sFilename)
    size_orig = myimg_orig.size
    parsings = seg_faces(
        myimg_orig, 
        np.array([
            [0,0,myimg_orig.size[0], myimg_orig.size[1]]
    ...
    res_img = myimg_orig.copy()
    res_img = res_img.resize((parsing.shape[1], parsing.shape[0]), Image.BICUBIC)

    vis_parsing_maps(res_img, parsing, size_orig, stride=1, save_im=True, save_path=rFilename)
    
    return rFilename

def _gan_image(sFilename, mFilename, rFilename):
    myimg_orig = Image.open(sFilename)
    inpaint_mask = Image.open(mFilename)
    orig_size = myimg_orig.size
    
    myimg_orig = myimg_orig.resize((512,512), Image.BICUBIC)
    inpaint_mask = inpaint_mask.resize((512,512), Image.BICUBIC)
    myimg_orig = np.asarray(myimg_orig)
    inpaint_mask = np.asarray(inpaint_mask)

    res_img = inpaint_blank(myimg_orig, inpaint_mask)
    res_img = Image.fromarray(res_img)
    res_img = res_img.resize(orig_size, Image.BICUBIC)
    res_img.save(rFilename)
    return rFilename
